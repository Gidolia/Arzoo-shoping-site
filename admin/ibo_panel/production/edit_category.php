...
?>
            ...
            <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data" method="post">
                    <div class="item form-group">
                        ...
                        <label class="col-form-label col-md-3 col-sm-3 label-align" for="first-name" va>Banner Image <span class="">*</span>
                        </label>
                        <div class="col-md-6 col-sm-6 ">
                            <img src="./images/<?php echo $fe['banner_image'];?>" height="100px" style="border-radius: 10px;">
                          <input type="file" name="new_img1"  class="form-control " >
                          <input type="hidden" value="<?php echo $fe['banner_image'];?>" name="old1">
                        </div>
                      </div>
                        <div class="ln_solid"></div>
                    <div class="item form-group">
                        <div class="col-md-6 col-sm-6 offset-md-3">
                        <button class="btn btn-primary" type="reset">Reset</button>
                          <button type="submit" name="banner_img" class="btn btn-success">Change</button>
                        </div>
                      </div>
...
if(isset($_POST[banner_img])){
        $id=rand(1000,99999);
        $old=$_POST[old1];
        $Mypic=$_FILES['new_img1'];
        $filename="banner".$id.".jpg";
        $dir="./images/";
        $NewLocation=$dir.$filename;
        if(move_uploaded_file($Mypic['tmp_name'], $NewLocation)===TRUE){
            $rs=$con->query("UPDATE `category` SET `banner_image`='$filename'  WHERE `id`='$_GET[id]'");
            if($rs===TRUE){
                unlink('images/'.$old);
                echo "<script>location.href='addCategory.php?m=es';</script>";
            }else{
