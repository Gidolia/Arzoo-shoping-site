...
if($con->query($res)===TRUE AND $con->query($upd)===TRUE ){
    echo "<script>alert('quantity updated');location.href='cart.php';</script>";
}else{
    $fail="INSERT INTO `error_fail_report` (`efr_id`, `date`, `time`, `c_id`, `admin_id`, `url`, `failed_query_n`) VALUES (NULL, '$date', '$time', '$_SESSION[id]', '', '$url', 'Quantity update Query');";
    	        $con->query($fail);
            echo "<script>alert('Quantity Update Failed plz try again'); location.href='cart.php';</script>";
}
...
if($res===TRUE ){
    echo "<script>alert('Product Removed');location.href='cart.php';</script>";
}else{
    $fail="INSERT INTO `error_fail_report` (`efr_id`, `date`, `time`, `c_id`, `admin_id`, `url`, `failed_query_n`) VALUES (NULL, '$date', '$time', '$_SESSION[id]', '', '$url', 'Quantity update Query');";
    	        $con->query($fail);
            echo "<script>alert('Product Remove Failed plz try again'); location.href='cart.php';</script>";
}
